...
.controller('DeuxmillecolsCtrl', function($state, $scope, $rootScope, $ionicLoading, homeFactory) {
  ...
  .then(function (response) {
    ...
    for (var i = 0; i < result_cols.length; i ++){
      ...
      $rootScope.json_result.push({cols_title:title_result, name0:name0, altitude0:altitude0, vertical0:vertical0, length0:length0, avggradient0:avggradient0, name1:name1, altitude1:altitude1, vertical1:vertical1, length1:length1, avggradient1:avggradient1, name2:name2, altitude2:altitude2, vertical2:vertical2, length2:length2, avggradient2:avggradient2, name3:name3, altitude3:altitude3, vertical3:vertical3, length3:length3, avggradient3:avggradient3 });
    }
...
.controller('DeuxmillecolsDetailCtrl', function($state, $scope, $rootScope, $ionicModal, $ionicSlideBoxDelegate, homeFactory) {
  ...
  .then(function (response) {
    ...
    for(var i = 0; i < result.length; i ++){
      $rootScope.json_member_result.push({member_name:result[i].name, member_avatar:result[i].avatar});
    }
  });
...
.controller('ChooseMemberCtrl', function($state, $scope, $rootScope, $ionicLoading, homeFactory) {
  ...
  .then(function (response) {
    ...
    $rootScope.json_member_result.push({member_name:result_name, member_avatar:result_avarta});
  });
...
.controller('CalculatorCtrl', function($scope, $rootScope) {
  ...
  $scope.powerChange = function(val){
    ...
    $scope.data.speedSelection = ''+parseInt($scope.NewtonRaphson(a, c, e, d));
    var V = $scope.NewtonRaphson(a, c, e, d);
    var temp_V = (V + vh) / 3.6;
  ...
  $scope.NewtonRaphson = function(a, c, e, d){
    var vi = 30;
    var i = 1;
    var converged = false;
...
.controller('ProfileCtrl', function($scope, $rootScope, $ionicModal, homeFactory) {
  ...
  .then(function (response) {
    ...
    if(result.status == "ok"){   
      ...
      $scope.conquredCols = result.user_cols_data.length;
    }else{
      alert("User Login failed!")
    }
  });
  ...
  $scope.newData = {units:'', ftp:'', weight:'', bikeweight:'', pwr:''};
